...
class LargeDivider : CCNode {
    ...
    //var CCparticle : CCParticleSystem! Skipped this particle... oops
    ...
    func moveParticles(amount: CGFloat) {
        for particle in particleArray {
            if particle.position.x > 0 && particle.position.x < 270 {
            ...
            else if particle.position.y > 0 && particle.position.y < 10 {
                ...
                else if particle.position.x == 0 && particle.position.y == 10 {
                    particle.position = ccp(particle.position.x + amount, particle.position.y)
                }
                else if particle.position.x == 270 && particle.position.y == 10 {
                    particle.position = ccp(particle.position.x, particle.position.y - amount)
                }
                else {
                    particle.position = ccp(particle.position.x - amount, particle.position.y)
            ...
            else if particle.position.x > 270 {
                particle.position = ccp(270, particle.position.y)
            }
            
            if particle.position.y < 0 {
                particle.position = ccp(particle.position.x, 0)
            }
            else if particle.position.y > 270 {
                particle.position = ccp(particle.position.x, 270)
            }
        }
    }
}